...
data_Jap=subset.data.frame(data, data$general_category == "Japanese" & data$Total_hour!=0)
```

```{r 90%}
train90.rows <- sample(rownames(data_Jap), dim(data_Jap)[1]*0.8)
train90 <- data_Jap[train90.rows,]

test90.rows <- sample(rownames(data_Jap), dim(data_Jap)[1]*0.2)
test90 <- data_Jap[test90.rows,]
```
