...
data_ASI=subset.data.frame(data, data$general_category == "Asian Fusion" & data$Total_hour!=0)
```

```{r 90%}
train90.rows <- sample(rownames(data_ASI), dim(data_ASI)[1]*0.8)
train90 <- data_ASI[train90.rows,]

test90.rows <- sample(rownames(data_ASI), dim(data_ASI)[1]*0.2)
test90 <- data_ASI[test90.rows,]
```
...
x2_pvalue <- 1- pchisq(X2, 14237)
c(X2 = X2, pvalue = x2_pvalue)
```

```{r accuracy90}
pred_prob11<- predict(m1, type = "response", newdata = test90 )
